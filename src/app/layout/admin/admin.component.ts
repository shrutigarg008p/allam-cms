...
export class AdminComponent implements OnInit {
  ...
  constructor(public menuItems: MenuItems, private router: Router, private authenticationService: AuthenticationService, private userService: UserService) {
  ...
  checkPermission(menu = '') {
    ...
    const userRoleId = this.loginUser['user'][0]['role_id'];
    const userPermission = this.loginUser.permission;
    ...
    this.isAuth = false;
    userPermission.forEach((permission, index) => {
  ...
  unread_requestChat() {
    this.userService.notificationUnread().subscribe((res: any) => {
      this.notificationUnread = res.data
    })
  }
